...
export function AutonomyLadder({
  ...
  return (
    <div
      className={cn(
        "gn-deployment-ladder grid gap-4",
        layout === "contact-split" ? "md:grid-cols-2" : "lg:grid-cols-4"
      )}
      data-layout={layout}
    >
      {steps.map((step, index) => (
        <div
          key={step.title}
          className="gn-panel relative flex h-full min-w-0 flex-col px-6 py-7"
        >
          ...
          <p className="mt-5 w-fit max-w-full self-start rounded-lg border border-border/70 bg-background/45 px-3 py-1 font-mono text-xs leading-5 text-muted-foreground">
