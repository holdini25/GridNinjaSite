...
import { enforceLeadRetention } from "@/lib/contact/operations"
...
export const POST = withVerifiedQstashSignature(async () => {
  ...
  try {
    const result = await enforceLeadRetention(deliveryOperationsRepository)

    return Response.json({ ok: true, ...result })
  } catch (error) {
    const errorCode = classifyContactError(error)

    logContactEvent("error", "lead_retention_failed", {
      errorCode,
