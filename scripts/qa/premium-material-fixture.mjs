...
import { createCameraForFrame, applySpecimenPose, inspectionCamera } from "/runtime/inspection-camera"
...
function replaceChunk(source, anchor, replacement) {
  if (source.split(anchor).length !== 2) throw new Error(`premium_diagnostic_shader_anchor:${anchor}`)
...
function diagnosticMaterials(mode) {
  ...
  for (const material of diagnostics) {
    ...
    material.onBeforeCompile = shader => {
      ...
      if (mode === "gray") shader.fragmentShader = replaceChunk(shader.fragmentShader, "#include <map_fragment>", "#include <map_fragment>\ndiffuseColor.rgb = vec3(0.25);")
      else {
        const value = mode === "normal" ? "normalize(normal) * 0.5 + 0.5" : mode === "roughness" ? "vec3(roughnessFactor)" : mode === "metalness" ? "vec3(metalnessFactor)" : mode === "ao" ? "vec3(gnDiagnosticAo)" : "gnDiagnosticUv"
        if (mode === "ao") shader.fragmentShader = replaceChunk(shader.fragmentShader, "#include <aomap_fragment>", `#include <aomap_fragment>
float gnDiagnosticAo = 1.0;
#ifdef USE_AOMAP
  gnDiagnosticAo = (texture2D(aoMap, vAoMapUv).r - 1.0) * aoMapIntensity + 1.0;
#endif`)
        if (mode === "uv") shader.fragmentShader = replaceChunk(shader.fragmentShader, "#include <map_fragment>", `#include <map_fragment>
vec3 gnDiagnosticUv = vec3(1.0, 0.0, 1.0);
#ifdef USE_MAP
  vec2 gnCell = floor(vMapUv * 32.0);
  float gnChecker = mod(gnCell.x + gnCell.y, 2.0);
  gnDiagnosticUv = mix(vec3(0.15), vec3(0.85), gnChecker);
#endif`)
        if (shader.fragmentShader.split("#include <opaque_fragment>").length !== 2) throw new Error("premium_diagnostic_shader_anchor")
        shader.fragmentShader = replaceChunk(shader.fragmentShader, "#include <opaque_fragment>", `gl_FragColor = vec4(${value}, diffuseColor.a); return;`)
      }
    }
...
window.captureAssetMaterial = async ({ kind, pose, mode, dpr, cssSize = [1020, 600] }) => {
  if (!["final", "gray", "normal", "roughness", "ao", "metalness", "uv"].includes(mode)) throw new Error("premium_diagnostic_mode")
  width = cssSize[0]; height = cssSize[1]
  restoreMaterials()
  if (currentKind !== kind) {
  ...
  camera = createCameraForFrame(inspectionCamera(model, view, width, height, width < 640))
  ...
  return {
    ...
    camera: { projection: camera.isPerspectiveCamera ? "perspective" : "orthographic", position: camera.position.toArray(), fov: camera.fov ?? null, frustum: camera.isOrthographicCamera ? [camera.left, camera.right, camera.top, camera.bottom] : null },
    materialNames: [...new Set([...originals.values()].flat().map(material => material.name))],
    interpretation: mode === "normal" ? "View-space perturbed normals encoded as RGB, before tone mapping." : mode === "roughness" ? "Final scalar roughness, including authored map and factor, encoded directly as grayscale." : mode === "ao" ? "Baked occlusion with runtime AO intensity; directly encoded grayscale." : mode === "metalness" ? "Final scalar metalness map times factor; directly encoded grayscale." : mode === "uv" ? "UV0 32-cell checker; magenta means no map UV." : mode === "gray" ? "Neutral diffuse geometry with fixed roughness, AO and masked coverage retained." : "Production PBR materials and authored lighting.",
